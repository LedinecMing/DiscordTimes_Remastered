use super::{defs::*, form::Form, rect::*, wrappers::Slider};
use crate::wrappers::SliderBuilder;
use derive_builder::Builder;
use notan::{
    app::{Event, Texture},
    draw::*,
    prelude::{App, AppState, Assets, Color, Graphics, Plugins},
};
use std::{
    collections::HashMap,
    fmt::{Debug, Formatter},
...
pub fn container<State: UIStateCl, T: PosForm<State>>(
    inside: Vec<T>,
) -> ContainerBuilder<State, T> {
    ContainerBuilder::default().inside(inside)
}
impl<State: UIStateCl, T: PosForm<State>> Form<State> for Container<State, T> {
    fn draw(
        ...
        self.calc_insides()
            .iter_mut()
            .for_each(|form| form.draw(app, assets, gfx, plugins, state, draw));
    }
    fn after(
        &mut self,
        app: &mut App,
        ...
        let inside_sizes = self
            .inside
            .iter()
            .map(|form| form.get_size())
            .collect::<Vec<Size>>();
        self.inside
            .iter_mut()
            .zip(0..inside_len)
            .for_each(|(inside, i): (&mut T, usize)| {
                let sizes = &inside_sizes[0..i];
                let interval = self.interval * i as f32;
                let pos = self.pos;
                let to_add = match self.align_direction {
                    Direction::Right => Position(
                        sizes.iter().map(|&size| size.0).sum::<f32>() + interval.0 + pos.0,
                        pos.1 + interval.1,
                    ),
                    ...
                    Direction::Top => Position(
                        pos.0 + interval.0,
                        -sizes.iter().map(|&size| size.1).sum::<f32>() + interval.1 + pos.1,
                    ),
...
impl<State: UIStateCl, T: PosForm<State>> Container<State, T> {
    pub fn calc_insides(&self) -> Vec<T> {
        ...
        self.inside
            .iter()
            .zip(0..(self.inside.len()))
            .map(|(inside, i): (&T, usize)| {
                let sizes = &inside_sizes[0..i];
                let interval = self.interval * i as f32;
                let to_add = match self.align_direction {
                    Direction::Right => Position(
                        sizes.iter().map(|&size| size.0).sum::<f32>() + interval.0 + self.pos.0,
                        self.pos.1 + interval.1,
                    ),
                    Direction::Left => Position(
                        -sizes.iter().map(|&size| size.0).sum::<f32>() + interval.0 + self.pos.0,
                        self.pos.1 + interval.1,
                    ...
                    Direction::Top => Position(
                        self.pos.0 + interval.0,
                        -sizes.iter().map(|&size| size.1).sum::<f32>() + interval.1 + self.pos.1,
                    ),
                    Direction::Bottom => Position(
                        self.pos.0 + interval.0,
                        sizes.iter().map(|&size| size.1).sum::<f32>() + interval.1 + self.pos.1,
...
impl<State: UIStateCl, T: PosForm<State>> Positionable for Container<State, T> {
    ...
    fn get_size(&self) -> Size {
        ...
        Size(
            match self.align_direction {
                Direction::Right => summed_h + interval.0,
                Direction::Left => -summed_h + interval.0,
                _ => sizes_v.max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
            },
            match self.align_direction {
                Direction::Top => -summed_v + interval.1,
                Direction::Bottom => summed_v + interval.1,
...
pub fn single<State: UIStateCl, T: PosForm<State>>(form: T) -> SingleContainerBuilder<State, T> {
    SingleContainerBuilder::default().inside(form)
}
...
pub fn slider<State: UIStateCl, T: PosForm<State>, K: PosForm<State>>(
    form: T,
    slider_inside: K,
    slider_rect: Rect,
    max_scroll: f32,
) -> SliderContainerBuilder<State, T, K> {
    SliderContainerBuilder::default().inside(form).slider(
...
impl<State: UIStateCl, T: PosForm<State>, K: PosForm<State>> Form<State>
    for SliderContainer<State, T, K>
{
    fn draw(
        &mut self,
        app: &mut App,
        ...
        self.inside
            .with_pos(Position(0., self.slider.scroll))
    ...
    fn after(
        ...
        let inside_rect = Rect {
            pos: self.inside.get_pos(),
            size: self.inside.get_size(),
        };
        let slider_rect = Rect {
            pos: self.slider.slider_inside.get_pos(),
            size: self.slider.slider_inside.get_size(),
        };
...
impl<State: UIStateCl, T: PosForm<State>, K: PosForm<State>> Positionable
    for SliderContainer<State, T, K>
{
    fn with_pos(&self, to_add: Position) -> Self {
        Self {
            inside: self.inside.clone(),
            slider: self.slider.with_pos(to_add),
...
pub fn straight_dyn<State: UIStateCl>(
    forms: Vec<Box<dyn ObjPosForm<State>>>,
) -> StraightDynContainerBuilder<State> {
    StraightDynContainerBuilder::default().inside(forms)
}
impl<State: UIStateCl> Form<State> for StraightDynContainer<State> {
    ...
    fn after(
        ...
        self.inside
            .iter_mut()
            .for_each(|inside: &mut Box<dyn ObjPosForm<State>>| {
...
pub fn dyn_cont<State: UIStateCl>(
    forms: Vec<Box<dyn ObjPosForm<State>>>,
) -> DynContainerBuilder<State> {
    DynContainerBuilder::default().inside(forms)
}
impl<State: UIStateCl> Form<State> for DynContainer<State> {
    fn draw(
        ...
        let inside_sizes = self
            .inside
            .iter()
            .map(|form| form.get_size_obj())
            .collect::<Vec<Size>>();
        self.inside.iter_mut().zip(0..inside_len).for_each(
            |(inside, i): (&mut Box<dyn ObjPosForm<State>>, usize)| {
                let sizes = &inside_sizes[0..i];
                let interval = self.interval * i as f32;
                let pos = self.pos;
                let to_add = match self.align_direction {
...
impl<State: UIStateCl> Positionable for DynContainer<State> {
    ...
    fn get_size(&self) -> Size {
        ...
        Size(
            size.0 + self.interval.0 * self.inside.len() as f32,
            size.1 + self.interval.1 * self.inside.len() as f32,
        )
    }
    fn get_pos(&self) -> Position {
        self.pos
...
pub fn center<State: UIStateCl, F: PosForm<State>>(
    v_h: (bool, bool),
    inside: F,
) -> Centered<State, F> {
    Centered {
        vertical_center: v_h.0,
        horizontal_center: v_h.1,
        inside,
        boo: Default::default(),
    }
}
...
impl<State: UIStateCl, F: PosForm<State>> Positionable for Centered<State, F> {
    ...
    fn get_pos(&self) -> Position {
        let size = self.inside.get_size();
        self.inside.get_pos() - Position(size.0 / 2., size.1 / 2.)
    }
