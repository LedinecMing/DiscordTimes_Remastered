...
use notan::draw::{Draw, DrawTransform, DrawBuilder, DrawImages, Image};
...
impl MovementChange {
	pub fn new(from: (f32, f32), to: (f32, f32)) -> Self { Self { from, to } }
}
#[derive(Clone, Debug)]
...
impl<'a> Animation {
	pub fn new(pos: MovementChange, size: SizeChange, time: (Instant, Duration), flip: (bool, bool)) -> Self {
		Self {
			pos, size, time, flip
		}
	}
	fn draw_process(&self, draw: &mut DrawBuilder<Image<'a>>) {
		...
		let size = (size_f.0 + diff_size.0 * elapsed, size_f.1 + diff_size.1 * elapsed);
		draw
