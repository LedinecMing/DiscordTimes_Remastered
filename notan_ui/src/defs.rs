...
use super::{form::Form, rect::*};
use derive_builder::UninitializedFieldError;
use dyn_clone::clone_box;
use notan::{
    draw::*,
    prelude::{App, AppState, Assets, Graphics, Plugins},
};
use std::fmt::Debug;
...
pub fn get_mut<T, K>(from: &mut T) -> &mut K
where
    T: Access<K>,
{
    Access::<K>::get_mut(from)
}
pub fn get<T, K>(from: &T) -> &K
where
    T: Access<K>,
{
    Access::<K>::get(from)
...
impl<T: Positionable> PartPositional for T {
    ...
    fn get_size_obj(&self) -> Size {
        Positionable::get_size(self)
    }
    fn get_pos_obj(&self) -> Position {
        Positionable::get_pos(self)
    }
}
...
pub type DrawFunction<State: UIStateCl, Form: PosForm<State>> =
    fn(&mut Form, &mut App, &mut Assets, &mut Graphics, &mut Plugins, &mut State, &mut Draw);
pub type UpdateFunction<State: UIStateCl, Form: PosForm<State>> =
    fn(&mut Form, &mut App, &mut Assets, &mut Plugins, &mut State);
...
impl From<UninitializedFieldError> for StructBuildError {
    fn from(ufe: UninitializedFieldError) -> StructBuildError {
        StructBuildError(ufe.to_string())
    }
}
...
macro_rules! impl_trytowith {
    ($typ:tt, $typ1:tt) => {
        impl<State> TryToWith<State, $typ1> for $typ
        where
            $typ: TryInto<$typ1> + Into<$typ1>,
        {
            fn with_to(&self, _state: &State) -> $typ1 {
                <Self as Into<$typ1>>::into(self)
            }
            fn try_to(&self) -> Option<$typ1> {
                <Self as TryInto<$typ1>>::try_into(self).ok()
            }
        }
    };
}
macro_rules! impl_trytome {
    ($typ:tt) => {
        impl<State> TryToWith<State, $typ> for $typ {
            fn with_to(&self, _state: &State) -> $typ {
                *self
            }
            fn try_to(&self) -> Option<$typ> {
                Some(*self)
            }
        }
        impl<State> TryToWith<State, Option<$typ>> for $typ {
            fn with_to(&self, _state: &State) -> Option<$typ> {
                Some(*self)
            }
            fn try_to(&self) -> Option<Option<$typ>> {
                Some(Some(*self))
            }
        }
    };
