								...
								match &attr.meta {
									syn::Meta::List(ml) => {
										ml.parse_nested_meta(|meta| {
											fields.last_mut().unwrap().aliases.1.push(meta.path.get_ident().unwrap().clone());
											Ok(())
										}).ok();
									},
									syn::Meta::Path(p) => {
								...
								if let Meta::NameValue(meta) = &attr.meta {
									if let syn::Expr::Lit(exprlit) = &meta.value {
										if let syn::Lit::Str(litstr) = &exprlit.lit {
											 fields.last_mut().unwrap().default = Some(litstr.parse::<Expr>().unwrap());
										}
									}
								}
							}
...
fn to_section_body(fields: &Vec<FieldInfo>, _ident: &Ident) -> TokenStream {
	let fields_filtered = fields
		.iter()
		.filter(|f| f.used);
	let fields_declarations = fields_filtered.clone()
		.map(|f| {
