...
use std::{collections::HashMap, fs::{self, DirEntry, FileType, ReadDir}, io::{self, Error}, os, path::Path};
mod locale;
use locale::*;
...
fn parse_mod_info(entry: Result<DirEntry, Error>) -> io::Result<ModInfo> {
	...
	if !meta.is_dir() { return io::Result::Err(io::Error::new(io::ErrorKind::Other, "Expected a directory")); }
...
fn parse_maps() -> io::Result<Vec<MapInfo>> {
	...
	for entry in fs::read_dir("./maps/")? {
		...
		if entry.file_type()?.is_file() && entry.path().ends_with(".DTm") {
			maps.push(MapInfo { name: entry.file_name().into_string().unwrap() });
		}
	}
	Ok(maps)
}
...
fn parse_base_lang(translations: Vec<String>, locale: &mut Locale) {
	...
	if translations.contains(&lang_name) {
		parse_locale(&*format!("./translations/{}", &lang_name), &lang_name, locale);
	} else {
				...
				egui::ComboBox::from_label("Выбрать карту")
					...
					.show_ui(ui, |ui| {
						let maps = if let Some(cur_mod) = &self.cur_mod {
							&self.maps.iter().filter(|map| self.deps.get(&map.name).is_some_and(|v| v.0 == Some(cur_mod.name.clone())))
								.cloned().collect()
						} else { &self.maps };
						ui.selectable_value(&mut self.cur_map, None, "Не выбрана");
						for map in maps {
							ui.selectable_value(&mut self.cur_map, Some(map.name.clone()), map.name.clone());
						}
					});
				egui::ComboBox::from_label("Выбрать мод")
					.selected_text(self.cur_mod.clone().and_then(|v| Some(v.name)).unwrap_or("Автоподбор".to_string()))
					.show_ui(ui, |ui| {
						ui.selectable_value(&mut self.cur_mod, None, "Автоподбор");
						if self.cur_map.is_none() {
							for mod_info in &self.mods {
								ui.selectable_value(&mut self.cur_mod, Some(mod_info.clone()), mod_info.name.clone());
							}
						}
					});
				egui::ComboBox::from_label("Выбрать язык")
					...
					.show_ui(ui, |ui| {
						...
						for lang in &self.translations {
							ui.selectable_value(&mut self.cur_lang, Some(lang.clone()), lang.clone());
						}
