use crate::{
    battle::{
        army::{find_path, Army, TroopType}, battlefield::{handle_action, BattleInfo}, control::{Player, Players}, troop::Troop, BattleUnitPos
    }, map::{
        event::{execute_event, execute_event_as_player, DelayedEvent, Event, Events, Execute, Executions},
        map::GameMap,
        object::ObjectInfo,
    }, parse::SETTINGS, time::time::Time, units::unit::{Unit, UnitInfo, UnitInventory, UnitLvl, UnitPos, UnitStats}, Menu
};
use alkahest::*;
use log;
...
impl Executor {
	pub fn message_handler(&mut self, message: ClientMessage, player: usize) {
		...
		match message {
			Action(to) => {
				let Some(battle) = &mut self.battle else { return; };
				if !([battle.army1, battle.army2].contains(&player_army) && battle.active_unit.is_some_and(|unit| unit.army == player_army)) {
					return;
				}
	...
	pub fn tick(&mut self, units: &Vec<Unit>) {
		let is_paused = self.map.pause || self.players.iter().all(|player| self.map.armys.get(player.army).is_some_and(|army| !army.path.is_empty()) || player.execution_queue.is_empty());
		if is_paused {
			return;
		}
		...
		for i in 0..(self.events.len()) {
			let res = execute_event(i, &mut self.players, &mut self.map, &mut self.events, units, false);
			if let Some(res) = res {
		...
		self.execution_queue.extract_if(0.., |event| {
			if event.time <= self.map.time {
				if let Some(mut res) = execute_event(event.event, &mut self.players, &mut self.map, &mut self.events, units, false) {
					new.append(&mut res);
				}
				true
			} else { false }
			...
			if let Some(execute) = player.execution_queue.get(0) {
				let res = match execute {
					...
					Execute::StartBattle(with) => {
						self.battle = Some(BattleInfo::new(&mut self.map.armys, player.army, *with));
						true
					},
					Execute::Wait(until) => {
						until <= &self.map.time
