...
async fn load_assets(req_assets_list: &[(&str, Vec<String>)]) -> Assets {
	...
	else { panic!("{}", error_collector.join("\n")); };
	Assets::new(
		asset_names.into_iter()
			.zip(assets)
			.collect()
	)
}
async fn game_init() -> State {
	...
	let req_assets_items = parse_items(&settings.locale);
	let res = parse_units();
	if let Err(err) = res {
		error!("{}", err);
		panic!("{}", err);
	...
	State {
		...
		units,
		objects,
		ui: Ui { main: Menu::Main, stack: Vec::new() },
		game: Game::Single(Scenario { gamemap, battle: None, events })
	}
}
#[derive(Clone, Debug)]
struct Scenario {
...
async fn main() {
	...
	draw_text("Loading game assets...", 0., screen_height() / 2., 20., BLACK);
	next_frame().await;
