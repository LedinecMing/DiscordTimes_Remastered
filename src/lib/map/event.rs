...
impl Conditions {
	fn new(relative_time: bool, repeat: Option<Time>, activation_time: Time, if_event_executed: Option<usize>, armys_defeated: Option<Vec<usize>>, not_executed: Option<Vec<usize>>, flag_check: String, xp_req: Option<Cmp<u64>>, gold_req: Option<Cmp<u64>>, mana_req: Option<Cmp<u64>>, army_req: Option<Cmp<u64>>, power_req: Option<Cmp<u64>>, hero_has_1_hp: bool, in_building: Option<usize>) -> Self {
		Self {
			relative_time,
			executed: false,
			repeat,
...
impl Event {
	fn new(name: String, player: Vec<usize>, location: Location, conditions: Conditions, result: EventResult, message: Option<String>) -> Self {
		Self {
			name,
			player,
			location,
...
pub fn execute_event(event: usize, gamemap: &mut GameMap, events: &mut Vec<Event>, units: &HashMap<usize, Unit>, executed_as_sub: bool) -> Option<Vec<Execute>> {
	{
		let Event { name, player, location, conditions, result, message } = &events[event];
		match location {
			Location::Sub => {
				if !executed_as_sub {
					return None;
		...
		if !(((!conditions.executed || conditions.repeat.is_some()) && conditions.activation_time <= gamemap.time)
			 && (conditions.if_event_executed.is_some_and(|event| events[event].conditions.executed) || conditions.if_event_executed.is_none())
			 && (conditions.armys_defeated.as_ref().is_some_and(|armys_index| armys_index.iter().all(|army| gamemap.armys[*army].defeated)) || conditions.armys_defeated.is_none())
			 && (conditions.not_executed.as_ref().is_some_and(|events_index| events_index.iter().all(|event| !events[*event].conditions.executed)) || conditions.not_executed.is_none())) {
			return None;
		}
	}
	let Event { name, player, location, conditions, result, message } = &mut events[event];
	let mut res = Vec::new();
	for player in player {
		if let Some(events) = execute_event_as_player(message, result, conditions, location, gamemap, *player, units) {
			res.extend(events);
		};
	}
	Some(res)
}
pub fn execute_event_as_player(message: &Option<String>, result: &mut EventResult, conds: &mut Conditions, location: &Location, gamemap: &mut GameMap, player: usize, units: &HashMap<usize, Unit>) -> Option<Vec<Execute>> {
	match location {
	...
	if (conds.xp_req.as_ref().is_some_and(|req| req.check(gamemap.armys[player].troops[0].get().unit.lvl.xp)) || conds.xp_req.is_none())
		&& (conds.gold_req.as_ref().is_some_and(|req| req.check(gamemap.armys[player].stats.gold)) || conds.gold_req.is_none())
		&& (conds.army_req.as_ref().is_some_and(|req| req.check(gamemap.armys[player].troops.len() as u64)) || conds.army_req.is_none())
		&& (conds.mana_req.as_ref().is_some_and(|req| req.check(gamemap.armys[player].stats.mana)) || conds.mana_req.is_none())
		&& (conds.power_req.as_ref().is_some_and(|req| true) || conds.power_req.is_none())
		&& (conds.hero_has_1_hp || !conds.hero_has_1_hp)
		&& (conds.in_building.and_then(|building| Some(gamemap.armys[player].building == building.into()))).unwrap_or(true)
	{
		let repeat = conds.repeat;
		// Player army items change
		if let Some(remove_items) = &mut result.minus_items {
			remove_items.iter().for_each(|item| gamemap.armys[player].remove_item(*item));
		}
		if let Some(add_items) = &mut result.plus_items {                                 
			add_items.iter().for_each(|item| gamemap.armys[player].add_item(*item));
		}
		...
		if let Some(event) = &result.sub_event {
			for event in event {
				res.push(Execute::Execute(DelayedEvent::new(Time::new(0), *event), player));
			}
		}
