...
pub fn execute_event(event: usize, gamemap: &mut GameMap, player: usize, events: &mut Vec<Event>, units: &HashMap<usize, Unit>, executed_as_sub: bool) -> Option<Vec<Execute>> {
	...
	if ((!conds.executed || conds.repeat.is_some()) && conds.activation_time <= gamemap.time)
		...
		&& (conds.xp_req.as_ref().is_some_and(|req| req.check(gamemap.armys[player].troops[0].get().unit.lvl.xp)) || conds.xp_req.is_none())
	...
	{
		let message = events[event].message.clone();
		let repeat = events[event].conditions.repeat;
		let result = &mut events[event].result;
